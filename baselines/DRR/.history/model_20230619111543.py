...
class EEPredictor:
    ...
    def predict(self):
        f = open("/home/hongyi/EMNLP_2023/id2Grade.json","r")
        id2grade = json.load(f)
        
        for grade in [0,1,2]:
            
            for RelationType in [["显式"],["隐式"],["显式","隐式"]]:
                print("logic grade",grade)
                print("relation type",RelationType)
                test_data = [d for d in self.test_data if d["RelationType"]in RelationType and id2grade[d["ID"]]==grade]
